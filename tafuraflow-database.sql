...
begin
  ...
  return jsonb_build_object(
    ...
    'next_publish_at',case when r.menu_design_changed_at is null then null else r.menu_design_changed_at+interval '365 days' end,
    'can_publish',r.menu_design_changed_at is null or r.menu_design_changed_at<=now()-interval '365 days',
...
begin
  ...
  if not allow_override and r.menu_design_changed_at is not null and r.menu_design_changed_at>now()-interval '365 days' then
    raise exception 'Menu design can only be published once every 365 days. Next publish available on %',to_char(r.menu_design_changed_at+interval '365 days','DD Mon YYYY');
  end if;
  ...
  insert into public.audit_logs(restaurant_id,actor_id,action,entity_type,entity_id,details)
  values(target_restaurant,auth.uid(),'restaurant.menu_design_published','restaurant',target_restaurant,jsonb_build_object('version',version_no,'override',allow_override));
