...
begin
  ...
  if split_mode='equal' then
    ...
    select count(distinct (x.value)::uuid),count(*) into assigned_count,assignment_count from jsonb_array_elements(item_assignments) a cross join lateral jsonb_array_elements_text(coalesce(a.value->'item_ids','[]'::jsonb)) x;
    select count(*) into invalid_count from jsonb_array_elements(item_assignments) a cross join lateral jsonb_array_elements_text(coalesce(a.value->'item_ids','[]'::jsonb)) x
...
begin
  ...
  return jsonb_build_object(
    ...
    'hourly_sales',coalesce((select jsonb_agg(x order by x."hour") from (select extract(hour from p.recorded_at at time zone b.timezone)::integer as "hour",sum(coalesce(p.allocated_amount,p.amount)) sales,count(*) transactions from public.payments p where p.branch_id=b.id and p.business_date between date_from and date_to and p.status='recorded' group by 1) x),'[]'::jsonb),
