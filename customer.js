(function(){
  ...
  const nav={
    super_admin:[['dashboard.html','Overview'],['companies.html','Companies']],
  ...
  const titles={dashboard:'Overview',companies:'Companies',tables:'Tables & QR codes',orders:'Orders',menu:'Menu',staff:'Staff',finance:'Finances',settings:'Restaurant settings'};
  ...
  async function requireAuth(){const {data:{session}}=await db.auth.getSession();if(!session){location.replace('index.html');return null}const ctx=await context();if(!ctx?.profile)throw new Error('Your profile is not ready. Run the DineQR SQL file in Supabase.');return ctx}
